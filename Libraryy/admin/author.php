...
<div class="col-md-4"style="margin-top:30px" >
    <form action="" class="navbar-form " method="post" name="form1">
        <div>
            <input   type="text" name="search" placeholder="Search Author by Book ID">
            <input type="submit" name="submit" value="Go" class=" btn-default btn-primary">           
        </div>
    </form>
</div>
    </div>
    <div  style = "margin: 30px 50px 10px 50px"><h3>AUTHORS INFO<hr></h3>
    <?php
          if(isset($_POST["submit"]))
		  {
			$res = mysqli_query($connection, "call  getAuthors('$_POST[search]')");
        
			if(mysqli_num_rows($res)==0)
			{
				echo "Enter valid Bookid";
			}
			else
			{
